...
function AnimatedCamera({ isReady }: { isReady: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    camera.zoom = THREE.MathUtils.lerp(camera.zoom, targetZoom, Math.min(delta * speed, 0.1))
      ...
      <div className="flex flex-col justify-center items-center text-center gap-6 h-full overflow-hidden">
        ...
        >
          <div className="flex-1 min-h-[400px]">
            <Canvas shadows camera={{ fov: 70, zoom: initialZoom }} onCreated={onCreated}>
              ...
              <Float speed={0.8} scale={0.75} position={[0, 0.25, 0]} rotation={[0, 0, 0]} >
                <Model rotation={[0.3, 0, 0]} visible={isReady} />
              </Float>
              <OrbitControls ref={ref} enableZoom={false} enablePan={true} />
              <Environment frames={Infinity} resolution={256}>
                <Lightformers />
              </Environment>
              <AnimatedCamera isReady={isReady} />
            </Canvas>
          </div>
        </div>
      </div>
